...
| **Error `Script not found` tras Build** | `dist/index.js` no se encontraba en el deploy | Usar `npm install --production` antes de `tsc` eliminaba los tipos (`@types/*`), lo que hacía fallar la compilación silenciosamente o producía un `dist` incompleto. Se cambió a `npm install` completo antes de `npm run build`. |
