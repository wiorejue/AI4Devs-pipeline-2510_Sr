...
| **Error Build en Deploy: `tsc: command not found`** | `npm warn config production Use --omit=dev...` y `sh: line 1: tsc: command not found` | `npm install --production` omitía `typescript` (devDependency), impidiendo compilar. Se cambió a `npm install --include=dev` para forzar la instalación de herramientas de construcción en el servidor, ignorando configuración global de producción. |
