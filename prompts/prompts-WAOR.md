...
| **Error Build en Deploy: `tsc: command not found`** | `sh: line 1: tsc: command not found` | Restricted production context on EC2. Solution: Explicitly `npm install typescript`, compile using local binary `./node_modules/.bin/tsc`, and configure PM2 to start `dist/index.js` directly to avoid re-triggering npm build scripts. |
| **Error `TS2345` en controllers** | `src/presentation/controllers/candidateController.ts(20,29): error TS2345: Argument of type 'string | string[]' is not assignable to parameter of type 'string'.` | Se añadió un casting explícito `as string` a `req.params.id` en `candidateController.ts` y `positionController.ts` para resolver la ambigüedad del tipo de parámetro en Express. |
| **Error `Script not found` en PM2** | `Error: [ERROR] Script not found: .../backend/dist/index.js` | Este error era consecuencia del fallo en la compilación. Al solucionar los errores de TypeScript, el comando `tsc` genera correctamente la carpeta `dist/` y PM2 puede iniciar el servidor. |
| **Código Obsoleto en EC2** | `Already up to date` (pero con errores antiguos) | El comando `git pull` no actualizaba el working directory si el repo estaba en estado "detatched" o había cambios locales. Se añadió `git checkout main` y `git reset --hard origin/main` para forzar la sincronización. |
| **Pipeline no se dispara en Push** | El workflow no se ejecutaba al subir cambios a `main` | El trigger original solo cubría `pull_request`. Se añadió `push: branches: [main]` en `pipeline.yml` para asegurar que cada actualización de la rama principal inicie el despliegue. |
| **Error `Script not found` tras Build** | `dist/index.js` no se encontraba en el deploy | Usar `npm install --production` antes de `tsc` eliminaba los tipos (`@types/*`), lo que hacía fallar la compilación silenciosamente. Además, la falta de `rootDir: "src"` en `tsconfig.json` hacía que `tsc` creara una carpeta `dist/src/` si encontraba archivos en `prisma/`. Se fijó `rootDir` y se limitó el `include` a `src`. |
